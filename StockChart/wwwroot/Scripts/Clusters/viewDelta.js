class viewDelta extends viewVolumesSeparated {
    ...
    getLegendLine() {
        return { "Text": "Delta", "Value": this.parent.selectedCoumn.cumDelta };
    }
    ...
    draw(parent, ctx, view, mtx) {
        ...
        minCumDelta -= d;
        ctx.restore();
        this.DrawZebra(ctx, view.x, view.y, view.w, view.h, minCumDelta, maxCumDelta);
        this.drawVertical();
        ctx.save();
        this.ctx.beginPath();
        this.ctx.myRect(this.view);
        this.ctx.clip();
        var m = mtx.reassignY({ y1: minCumDelta, y2: maxCumDelta }, { y1: view.h + view.y, y2: view.y });
        function dr() {
            ctx.beginPath();
            for (var i = parent.minIndex; i <= parent.maxIndex; i++) {
                var y = parent.data.clusterData[i].cumDelta;
