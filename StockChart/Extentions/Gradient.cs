namespace StockChart.Extentions
{
    public static class Gradient
    {
        ...
        private static string GradientOverlayColor(int R, int G, int B, decimal min, decimal max, decimal val)
        {
            var v = vec(min, max, val);
        ...
        public static string GradientColorYRgba(decimal max, decimal val, int colorModel)
        {
            if (colorModel == 0)
                return (val > 0) ? GradientOverlayColor(4, 163, 68, 0, max, val) : GradientOverlayColor(214, 24, 0, 0, max, -val);
            return (val > 0) ? GradientOverlayColor(107, 165, 131, 0, max, val) : GradientOverlayColor(215, 84, 66, 0, max, -val);
        }
    }
}
