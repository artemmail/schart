...
import { VolfixColumn } from '../Columns/VolfixColumn';
...
export class viewMain extends canvasPart {
  ...
  onPanStart(e: any) {
    if (!this.parent.markupEnabled || this.parent.markupManager.allowPan()) {
      this.parent.translateMatrix = new Matrix().translate(
        e.deltaX * this.colorsService.scale(),
        e.deltaY * this.colorsService.scale()
      );
  ...
  onMouseMovePressed(e: Point) {
    if (this.parent.markupEnabled)
      this.parent.markupManager.onMouseDownMove(e);

      if (!this.parent.markupEnabled || this.parent.markupManager.allowPan()) 
        this.parent.translateMatrix = new Matrix().translate(
        - (this.parent.pressd.x - e.x)  * this.colorsService.scale(),
         - (this.parent.pressd.y - e.y) * this.colorsService.scale()
        );
  ...
  onMouseUp(e: Point) {
    if (this.parent.markupEnabled)
      this.parent.markupManager.onMouseUp(e);
 ...
 //   canvas.style.cursor = 'move';// selectedPoint == null ? (mode == 'Edit' ? 'move' : 'default') : 'pointer';
    ...
    var p = 
      Math.round(point.y / this.parent.data.priceScale) * this.parent.data.priceScale;
    let pp = this.formatService.drob(p, 4);
    ...
    this.drawHint(e.position);

    //if (e.button == 0) 
    this.parent.drawClusterView();
  }

  drawHint(offset: Point) {
    /*
    ...
    if (col.Number != 0) {
      ...
      data += item('Volume', this.formatService.MoneyToStr(col.v));
    } else {
      ...
      if (!FPsettings.ExtendedToolTip) {
        ...
        data += item(
          'Buy',
          `${col.bq} (${((col.bq * 100) / col.q).toFixed(1)}%)`
        );
      } else {
        if (col.bq > 0)
          data += item(
        ...
        if (col.q - col.bq > 0)
          data += item(
            'Sell',
            `${col.q - col.bq} (${(((col.q - col.bq) * 100) / col.q).toFixed(
              1
            )}%)`
          );
      }
      ...
      for (let i = 0; i < col.cl.length; i++)
        if (col.cl[i].p - parseFloat(r) == 0) {
          ...
          if (!FPsettings.ExtendedToolTip) {
            ...
            data += item(
              'Buy',
              `${col.cl[i].bq} (${((col.cl[i].bq * 100) / col.cl[i].q).toFixed(
                1
              )}%)`
            );
          } else {
            ...
            if (col.cl[i].q - col.cl[i].bq > 0)
              data += item(
                'Sell',
                `${col.cl[i].q - col.cl[i].bq} (${(
                  ((col.cl[i].q - col.cl[i].bq) * 100) /
                  col.cl[i].q
                ).toFixed(1)}%)`
              );
          }
    ...
    if (hint != null) {
      ...
      hint.style.left = offset.x + 5 + 'px';
      hint.style.top = offset.y + 5 + 'px';
  ...
  ParmasFromCandle1(dt: Date, period: number) {
    ...
    var tickerparams = {
      startDate: this.formatService.removeUTC(dt),
      endDate: this.formatService.removeUTC(dt2),
      period: newperiod,
    };
    return tickerparams;
  }
  ...
  onDblClick(point: Point) {
    ...
    (cp.ticker = params.ticker), (cp.priceStep = params.priceStep);
  ...
  draw(parent: FootPrintComponent,  view: Rectangle, mtx: Matrix) {
    let params = this.parent.params;
    var FPsettings: ChartSettings = this.parent.FPsettings; let ctx = this.parent.ctx;
    ...
    if (
      ...
      switch (FPsettings.style) {
        case 'Ruticker':
          ...
          break;
        case 'Volume':
          ColumnBuilder = new VolfixColumn(parent, view, mtx);
          break;
        case 'ASKxBID':
