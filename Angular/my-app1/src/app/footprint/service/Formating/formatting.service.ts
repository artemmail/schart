...
export class FormattingService {
  ...
  public drob(v: number, sh: number = 2) {
    if (v == 0) return 0;
    let x = -(Math.round(Math.log10(Math.abs(v))) - sh);
    if (x < 0) x = 0;
    let p = Math.pow(10, x);
    return Number(Math.round(v * p) / p)
      .toFixed(10)
      .replace(/\.?0+$/, '');
  }

  public MoneyToStr(mon: number) {
    var mona = Math.abs(mon);
    let t = ~~(Math.log10(mona) / 3);
    if (t > 1)
      return (
        this.drob(mon / Math.pow(10, t * 3), 2) +
        ' ' +
        ['тыс', 'млн', 'млрд', 'трлн', 'блн', 'хул', '*', '*'][t - 1]
      );
    return this.drob(mon, 4);
  }
  ...
  public rrounder(x: number, r: number) {
